...
Team names change most years, so a name you see in one place may be stale in another. Two renames for the ${MFL_SEASON} season: "Tank U, Next" is now "I can buy myself flowers", and "and the Giant Peach" is now "Drinkers From Shoes" — same owners, same franchises. This matters in two specific places: the SQL database only goes to 2025, so it holds the OLD names; and MFL freezes team names into draft-pick comment strings at the moment of a trade, so draftResults still says things like "[Pick traded from Tank U, Next.]". Treat both old names as referring to the current teams, and use the current name when you answer. If any other team name in the history doesn't match a current one, resolve it through franchise_owners (the owner is the stable identity, not the team name) rather than treating it as an unknown team.
